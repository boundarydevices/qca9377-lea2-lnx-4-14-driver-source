...
#if ADD_VID
static struct {
	char *ptrn; /* Patterns 0-15 */
	v_U8_t vid; /*sessionId*/
} g_hdd_wowl_ptrns[WOWL_MAX_PTRNS_ALLOWED];
#else
...
/* silex added comment only.
 * ptrn is terminated with '\0' in hdd_apply_cfg_ini() because of WLAN_PARAM_String.
 * we don't care the size for ptrn in find_ptrn_len().
 */
...
v_BOOL_t hdd_add_wowl_ptrn (hdd_adapter_t *pAdapter, const char * ptrn)
{
  ...
  while ( len >= 11 )
  {
    ...
    {
      if(g_hdd_wowl_ptrns[i].ptrn == NULL) continue;

      if(!memcmp(ptrn, g_hdd_wowl_ptrns[i].ptrn, len) && (sessionId == g_hdd_wowl_ptrns[i].vid))
      {
        // Pattern Already configured, skip to next pattern
        VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_ERROR,
            "Trying to add duplicate WoWL pattern. Skip it! vid=%d", sessionId);
        ptrn += len;
        goto next_ptrn;
      }
    }
    ...
    for (i=0; i<pHddCtx->cfg_ini->maxWoWFilters; i++)
    {
      if(g_hdd_wowl_ptrns[i].ptrn == NULL) {
        first_empty_slot = i;
        break;
      }
    }
    ...
    g_hdd_wowl_ptrns[first_empty_slot].ptrn = (char*) kmalloc(len+1, GFP_KERNEL);
    if(g_hdd_wowl_ptrns[first_empty_slot].ptrn == NULL)
    {
      VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_ERROR,
          "%s: kmalloc failure", __func__);
      return VOS_FALSE;
    }
    g_hdd_wowl_ptrns[first_empty_slot].vid = sessionId;

    memcpy(g_hdd_wowl_ptrns[first_empty_slot].ptrn, temp, len);
    g_hdd_wowl_ptrns[first_empty_slot].ptrn[len] = '\0';
    ...
    if ( !HAL_STATUS_SUCCESS( halStatus ) )
    {
      ...
      kfree(g_hdd_wowl_ptrns[first_empty_slot].ptrn);
      g_hdd_wowl_ptrns[first_empty_slot].ptrn = NULL;
    }
#else
    //All is good. Store the pattern locally
...
v_BOOL_t hdd_del_wowl_ptrn (hdd_adapter_t *pAdapter, const char * ptrn)
{
...
#if ADD_VID
  // Detect pattern
  for (id=0; id<pHddCtx->cfg_ini->maxWoWFilters && g_hdd_wowl_ptrns[id].ptrn != NULL; id++)
  {
    if(!strcmp(ptrn, g_hdd_wowl_ptrns[id].ptrn) && g_hdd_wowl_ptrns[id].vid == sessionId)
    {
  ...
  if(patternFound)
  {
    ...
    if ( HAL_STATUS_SUCCESS( halStatus ) )
    {
      ...
      VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_ERROR,
          "Deleted pattern with id %d [%s]", id, g_hdd_wowl_ptrns[id].ptrn);

      kfree(g_hdd_wowl_ptrns[id].ptrn);
      g_hdd_wowl_ptrns[id].ptrn = NULL;
      g_hdd_wowl_ptrns[id].vid = -1;
      return VOS_TRUE;
    }
  }
#else
...
v_BOOL_t hdd_init_wowl (hdd_adapter_t*pAdapter)
{
  ...
  if (init_complete == FALSE) {
    ...
    for (i=0; i < WOWL_MAX_PTRNS_ALLOWED; i++) {
		g_hdd_wowl_ptrns[i].vid = -1; /* set invalid ID */
	}
    init_complete = TRUE;
  }
  if (pAdapter->wdev.iftype == NL80211_IFTYPE_P2P_DEVICE)
