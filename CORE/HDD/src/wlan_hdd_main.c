...
void hdd_checkandupdate_phymode( hdd_context_t *pHddCtx)
{
   ...
   phyMode = sme_GetPhyMode(WLAN_HDD_GET_HAL_CTX(pAdapter));
    ...
    /* should not down 11ac -> 11n even VHT80 does not available.
        we change to that 11ac does availavle even VHT80 not available. It likes SX-PCEAC. */

	/* following code is dead code */
#else
...
/* Check the Country Code is related to ETSI or not. */
static int check_country_code_etsi(u_int8_t *alpha2)
{
    int32_t regdmn = 0;

    regdmn = regdmn_get_regdmn_for_country(alpha2);
    switch(regdmn) {
    case ETSI1_WORLD:
    case ETSI3_ETSIA:
    case ETSI2_WORLD:
...
void wlan_hdd_set_nflimit(hdd_context_t *pHddCtx)
{
    ...
    /* Check the Country Code is related to ETSI or not. */
    is_etsi = check_country_code_etsi(pHddCtx->reg.alpha2);

    if (pHddCtx->cfg_ini->nflimit_max2g || (is_etsi && pHddCtx->cfg_ini->nflimit_max2g_etsi)) {
    ...
    if (pHddCtx->cfg_ini->nflimit_max5g || (is_etsi && pHddCtx->cfg_ini->nflimit_max5g_etsi)) {
...
void wlan_hdd_set_thresh62(hdd_context_t *pHddCtx)
{
    ...
    if (pHddCtx->cfg_ini->thresh62_2g || (is_etsi && pHddCtx->cfg_ini->thresh62_2g_etsi)) {
    ...
    if (pHddCtx->cfg_ini->thresh62_5g || (is_etsi && pHddCtx->cfg_ini->thresh62_5g_etsi)) {
