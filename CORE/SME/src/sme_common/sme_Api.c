...
/* ---------------------------------------------------------------------------
    \fn sme_ClearCCKMIe
    \brief  function to clear CCKM Ie before requesting IE from supplicant.
    \param  hHal - HAL handle for device
    \param  sessionId - Session Identifier
    \- return Success or failure
  --------------------------------------------------------------------------*/
eHalStatus sme_ClearCCKMIe(tHalHandle hHal, tANI_U8 sessionId)
{
    tpAniSirGlobal pMac = PMAC_STRUCT( hHal );
    eHalStatus     status  = eHAL_STATUS_SUCCESS;
    tCsrRoamSession *pSession = CSR_GET_SESSION(pMac, sessionId);
    ...
    status = sme_AcquireGlobalLock( &pMac->sme );
    if ( HAL_STATUS_SUCCESS( status ) ){
        pSession->suppCckmIeInfo.cckmIeLen = 0;
        sme_ReleaseGlobalLock(&pMac->sme);
    }
    return status;
}
...
--------------------------------------------------------------------------- */
eHalStatus sme_ConfigureDTIMPolicy( tHalHandle hHal)
{
    eHalStatus status = eHAL_STATUS_SUCCESS;
    VOS_STATUS vosStatus = VOS_STATUS_SUCCESS;
    tpAniSirGlobal pMac = PMAC_STRUCT(hHal);
...
*/
    if ( eHAL_STATUS_SUCCESS == ( status = sme_AcquireGlobalLock( &pMac->sme ) ) )
    {
        ...
        vosMessage.type    = WDA_WLAN_DTIM_POLICY;
        vosStatus = vos_mq_post_message( VOS_MQ_ID_WDA, &vosMessage );
        if ( !VOS_IS_STATUS_SUCCESS(vosStatus) )
        {
           status = eHAL_STATUS_FAILURE;
      ...
      uint8_t channel, uint16_t *vht_channel_width)
{
   ...
   tANI_U8 i, startChan = channel, chanCnt = 0, chanBitmap = 0, centerChn = 0;
   ...
   switch (mode)
   {
      ...
      case eCSR_INI_DOUBLE_CHANNEL_LOW_PRIMARY:
         ...
         break;
#ifdef WLAN_FEATURE_11AC
         /* centerChn is not cared for 80MHz because it is for 40HMz by silex */
      case eCSR_INI_QUADRUPLE_CHANNEL_20MHZ_LOW_40MHZ_LOW:
         startChan = channel;
         chanCnt = 4;
         break;
   ...
   /* check for 40Mhz by silex */
   if (newMode == eCSR_INI_DOUBLE_CHANNEL_HIGH_PRIMARY
       || newMode == eCSR_INI_DOUBLE_CHANNEL_LOW_PRIMARY)
   {
       if(eANI_BOOLEAN_FALSE == csrRoamIsValid40MhzChannel(pMac, centerChn))
       {
                            ...
                            uint16_t ch_width_orig)
{
   ...
   *vht_channel_width = ch_width_orig;
   ...
   if ((eCSR_DOT11_MODE_11ac == eCsrPhyMode ||
      ...
      /* silex added checking process for channel width setting */
      if ((pMac->roam.configParam.nVhtChannelWidth == 1)
         && (smeConfig.csrConfig.channelBondingMode5GHz > eCSR_INI_DOUBLE_CHANNEL_LOW_PRIMARY )
         )
      {
         eAniBoolean ext, low_ch;

         /* check w58 band, 149, 153, 157, 161 and 165. */
         ext = ((channel % 4) != 0);

         if ( ext )
         {
            low_ch = (( channel - 145 ) / 4 ) % 2;

            /* no need to care 165ch(HT20) at this timing */
            if ( low_ch )
               smeConfig.csrConfig.channelBondingMode5GHz =
                  eCSR_INI_DOUBLE_CHANNEL_LOW_PRIMARY;
            else
