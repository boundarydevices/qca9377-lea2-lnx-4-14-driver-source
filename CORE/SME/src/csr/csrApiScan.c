    ...
    do
    {
        if(pMac->scan.fScanEnable)
        {
            ...
            if(pScanCmd)
            {
				...
				else if (pScanRequest->restTime == 0) {
						if(csrIsAnySessionConnected(pMac))
							pScanRequest->restTime = pMac->roam.configParam.nRestTimeConc;
				}
#endif
...
eHalStatus csrScanChannels( tpAniSirGlobal pMac, tSmeCmd *pCommand )
{
    ...
    do
    {
         ...
         */
        /* silex delete cached results when scaning start.
         * STAUT reconnect AP with invalid channel when AP change channel to 2ch from 3ch.
         * Cached results are deleted at scaning end and scaning start.
         */
        scanReq.freshScan = SIR_BG_SCAN_RETURN_FRESH_RESULTS | SIR_BG_SCAN_PURGE_RESUTLS;
