    ...
    if ((csrIsProfileESE(pProfile) ||
         (HAL_STATUS_SUCCESS(status) && (pIesTemp->ESEVersion.present)
          && ((pProfile->negotiatedAuthType == eCSR_AUTH_TYPE_OPEN_SYSTEM)
              ...
              || (pProfile->negotiatedAuthType == eCSR_AUTH_TYPE_CCKM_WPA)
              || (pProfile->negotiatedAuthType == eCSR_AUTH_TYPE_CCKM_RSN)
#ifdef WLAN_FEATURE_11W
              || (pProfile->negotiatedAuthType == eCSR_AUTH_TYPE_RSN_8021X_SHA256)
        ...
        else if(eWNI_SME_REASSOC_REQ == messageType )
        {
            ...
            if( csrIsProfileESE( pProfile ) )
            {
                ...
                if(ieLen > 0)
                    ...
                    smsLog(pMac, LOGW, FL("Warning - ESE profile but no CCKM IE\r\n"));
                }
  ...
  if ( NULL != pSession->pCurRoamProfile )
  {
    if ( pSession->pCurRoamProfile->csrPersona == VOS_STA_MODE )
    {
      return eANI_BOOLEAN_TRUE;
            ...
            smsLog(pMac, LOGW, FL(" %s: Not a STATION Mode\n"), __func__); //embwise 2016-11-28
            return eANI_BOOLEAN_FALSE;
    }
  }
                    ...
                    eUpdateIEsType updateType)
{
    ...
    pUpdateAddIEs->updateIE.notify = pUpdateIE->notify;/*silex add*/
